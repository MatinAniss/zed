...
impl LineLayoutCache {
        ...
        SharedString: From<Text>,
    {
        ...
        if let Some((key, layout)) = previous_frame_entry {
            ...
            let unwrapped_layout = self.layout_line::<&SharedString>(&text, font_size, runs, inline_boxes.unwrap_or_default(), None);
        ...
        SharedString: From<Text>,
    {
        ...
        if let Some((key, layout)) = self.previous_frame.lock().lines.remove_entry(key) {
            ...
            let mut inline_boxes_ascent = layout.ascent;
                ...
                for glyph in run.glyphs.iter_mut() {
                    while let Some(inline_box) = current_inline_box {
                        if inline_box.glyph_ix == global_glyph_ix {
                            ...
                            inline_boxes_ascent =
                                inline_boxes_ascent.max(inline_box.size.height + layout.ascent);
            ...
            layout.width += inline_boxes_width;
